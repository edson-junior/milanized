...
export default function Hero({
  ...
  return (
    <div
      className={cn(
        'group block mb-6 relative bg-no-repeat w-full py-8 lg:py-16 bg-[#212121]',
        className
      )}
    >
      <div className="text-white flex align-middle h-full flex-col justify-center max-w-7xl mx-auto px-4 py-4 relative">
        {typeof mainTitle === 'string' ? (
          <Heading
            as="h1"
            className="text-3xl lg:text-5xl [text-shadow:_1px_1px_1px_rgb(0_0_0_/_0.45)] mb-2 lg:mb-4"
          >
            {mainTitle}
          </Heading>
        ) : (
        ...
        {typeof subtitle === 'string' ? (
          <p className="leading-7 mb-8 lg:text-lg">{subtitle}</p>
        ) : (
          subtitle
        )}
        {children}
